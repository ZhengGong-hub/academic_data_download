...
df <- read_parquet("/home/ubuntu/academic_data_download/Analysis/imbalance.parquet")
...
df <- df %>% filter(prc >= 5) %>% 
  mutate(
    # Change date format
    ann_deemed_date = as.Date(ann_deemed_date, format = "%Y-%m-%d"), last_earnings_deemed_date = as.Date(last_earnings_deemed_date, format = "%Y-%m-%d"), next_earnings_deemed_date = as.Date(next_earnings_deemed_date, format = "%Y-%m-%d"),
    after_earning = ann_deemed_date - last_earnings_deemed_date,
    ...
    # Shift past CARs
    car_252d_ff1 = (car_252d_ff1+1)/(1+ar_ff1) - 1, car_126d_ff1 = (car_126d_ff1+1)/(1+ar_ff1) - 1, car_22d_ff1 = (car_22d_ff1+1)/(1+ar_ff1) - 1, car_1d_ff1 = (car_2d_ff1+1)/(1+ar_ff1) - 1,
    car_252d_ff3 = (car_252d_ff3+1)/(1+ar_ff3) - 1, car_126d_ff3 = (car_126d_ff3+1)/(1+ar_ff3) - 1, car_22d_ff3 = (car_22d_ff3+1)/(1+ar_ff3) - 1, car_1d_ff3 = (car_2d_ff3+1)/(1+ar_ff3) - 1,
    car_252d_ff6 = (car_252d_ff6+1)/(1+ar_ff6) - 1, car_126d_ff6 = (car_126d_ff6+1)/(1+ar_ff6) - 1, car_22d_ff6 = (car_22d_ff6+1)/(1+ar_ff6) - 1, car_1d_ff6 = (car_2d_ff6+1)/(1+ar_ff6) - 1
    
    )

summary(df$cum_ret_1d)
...
# Get some statistics
nrow(df) # 841,224
summary(df$direction)["Mean"] # mean: 0.8528668
...
df_summary <- df
vars_ret   <- c("cum_ret_252d", "cum_ret_126d", "cum_ret_22d", "cum_ret_1d", "fwd_car_1d", "fwd_car_2d", "fwd_car_3d", "fwd_car_4d", "fwd_car_5d", "fwd_car_22d", "fwd_car_126d", "fwd_car_252d")
...
df_regression <- df %>% filter(after_earning > days_after_earnings) %>% mutate(Post = ifelse(ann_deemed_date >= as.Date("2018-01-01"),1,0), bucket_linear = dplyr::ntile(linear, 5), bucket_estimate = dplyr::ntile(estimate, 5), bucket_price = dplyr::ntile(prc, 5),
...
df_regression$bucket_linear <- factor(df_regression$bucket_linear, levels = 1:5); df_regression$bucket_estimate <- factor(df_regression$bucket_estimate, levels = 1:5); df_regression$bucket_price <- factor(df_regression$bucket_price, levels = 1:5)
#; df_regression$bucket <- relevel(df_regression$bucket, ref = "3")

# Transform variables
vars_ret  <- c("cum_ret_252d", "cum_ret_126d", "cum_ret_22d", "cum_ret_1d", "car_252d_ff6", "car_126d_ff6", "car_22d_ff6", "car_1d_ff6", "fwd_ret_1d", "fwd_ret_2d", "fwd_ret_3d", "fwd_ret_4d", "fwd_ret_5d", "fwd_ret_22d", "fwd_ret_126d", "fwd_ret_252d", "fwd_car_1d_ff6", "fwd_car_2d_ff6", "fwd_car_3d_ff6", "fwd_car_4d_ff6", "fwd_car_5d_ff6", "fwd_car_22d_ff6", "fwd_car_126d_ff6", "fwd_car_252d_ff6",
               "car_252d_ff1", "car_126d_ff1", "car_22d_ff1", "car_1d_ff1", "fwd_car_1d_ff1", "fwd_car_2d_ff1", "fwd_car_3d_ff1", "fwd_car_4d_ff1", "fwd_car_5d_ff1", "fwd_car_22d_ff1", "fwd_car_126d_ff1", "fwd_car_252d_ff1",
               "car_252d_ff3", "car_126d_ff3", "car_22d_ff3", "car_1d_ff3", "fwd_car_1d_ff3", "fwd_car_2d_ff3", "fwd_car_3d_ff3", "fwd_car_4d_ff3", "fwd_car_5d_ff3", "fwd_car_22d_ff3", "fwd_car_126d_ff3", "fwd_car_252d_ff3",
               "ret", "ar_ff1", "ar_ff3", "ar_ff6")
...
df_regression <- standardise_vars(df_regression, setdiff(cols_to_winsor, c("imbalance_in_1d_100000_0", "imbalance_in_1d_500_0", "imbalance_in_1d_2000_500", "imbalance_in_1d_10000_2000", "imbalance_in_1d_100000_30000", "imbalance_in_1d_30000_10000",
                                                                           ...
                                                                           "fwd_car_1d_log_ff1", "fwd_car_2d_log_ff1", "fwd_car_3d_log_ff1", "fwd_car_4d_log_ff1", "fwd_car_5d_log_ff1", "fwd_car_22d_log_ff1", "fwd_car_126d_log_ff1", "fwd_car_252d_log_ff1",
                                                                           "fwd_car_1d_log_ff3", "fwd_car_2d_log_ff3", "fwd_car_3d_log_ff3", "fwd_car_4d_log_ff3", "fwd_car_5d_log_ff3", "fwd_car_22d_log_ff3", "fwd_car_126d_log_ff3", "fwd_car_252d_log_ff3",
                                                                           "fwd_car_1d_log_ff6", "fwd_car_2d_log_ff6", "fwd_car_3d_log_ff6", "fwd_car_4d_log_ff6", "fwd_car_5d_log_ff6", "fwd_car_22d_log_ff6", "fwd_car_126d_log_ff6", "fwd_car_252d_log_ff6",
                                                                           "ret_log", "ar_ff1_log", "ar_ff3_log", "ar_ff6_log")))

# Rescale variables
df_regression <- df_regression %>%  mutate(across(all_of(paste0(vars_ret, "_log")), ~ .x * 10000))

# Retail imbalance
im1 <- felm(imbalance_in_1d_100000_0 ~ linear | 0 | 0 | permno + ann_deemed_date, data = df_regression)
summary(im1)

im2 <- felm(imbalance_in_1d_100000_0 ~ linear +
...
im3 <- felm(imbalance_in_1d_100000_0 ~ linear + estimate +
              `imbalance_in_-1d_100000_0` + cum_ret_252d_log + cum_ret_126d_log + cum_ret_22d_log + cum_ret_1d_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(im3)

im4 <- felm(imbalance_in_1d_100000_0 ~ linear + estimate + prc_yest_log +
              `imbalance_in_-1d_100000_0` + cum_ret_252d_log + cum_ret_126d_log + cum_ret_22d_log + cum_ret_1d_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(im4)
...
# Robustness Check
#rownames(df_regression) <- as.character(seq_len(nrow(df_regression)))

# First stage regression
#fs_linear <- felm(linear ~ estimate + prc_yest_log | 0 | 0 | 0, data = df_regression)
#summary(fs_linear)

# Add residual to data frame
#fs_resid <- resid(fs_linear)
#rows_used <- as.integer(rownames(model.frame(fs_linear)))

# Create / overwrite the column
#df_regression$linear_pure <- NA_real_

# Fill residuals into the rows that were actually used
#df_regression$linear_pure[rows_used] <- fs_resid


# Second stage regression
ss_linear <- felm(imbalance_in_1d_100000_0 ~ linear*bucket_estimate + estimate + prc_yest_log +
    `imbalance_in_-1d_100000_0` + cum_ret_252d_log + cum_ret_126d_log + cum_ret_22d_log + cum_ret_1d_log +
    `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ss_linear)
...
# Now with CAR: FF1
ff1_0 <- felm(car_1d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_0)

summary(df_regression$car_1d_ff1) # Falsch
summary(df_regression$car_252d_ff1)



ff1_1 <- felm(fwd_car_1d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
               `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_1)

ff1_2 <- felm(fwd_car_2d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
               `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_2)

ff1_3 <- felm(fwd_car_3d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
               `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_3)

ff1_4 <- felm(fwd_car_4d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
               `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_4)

ff1_5 <- felm(fwd_car_5d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
               `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_5)

ff1_22 <- felm(fwd_car_22d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_22)

ff1_126 <- felm(fwd_car_126d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                 `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_126)

ff1_252 <- felm(fwd_car_252d_ff1_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                 `imbalance_in_-1d_100000_0` + car_1d_ff1_log + car_252d_ff1_log + car_126d_ff1_log + car_22d_ff1_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff1_252)


# Now with CAR: FF3
ff3_1 <- felm(fwd_car_1d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_1)

ff3_2 <- felm(fwd_car_2d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_2)

ff3_3 <- felm(fwd_car_3d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_3)

ff3_4 <- felm(fwd_car_4d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_4)

ff3_5 <- felm(fwd_car_5d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_5)

ff3_22 <- felm(fwd_car_22d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_22)

ff3_126 <- felm(fwd_car_126d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_126)

ff3_252 <- felm(fwd_car_252d_ff3_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff3_log + car_252d_ff3_log + car_126d_ff3_log + car_22d_ff3_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff3_252)


# Now with CAR: FF6
ff6_1 <- felm(fwd_car_1d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_1)

ff6_2 <- felm(fwd_car_2d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_2)

ff6_3 <- felm(fwd_car_3d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_3)

ff6_4 <- felm(fwd_car_4d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_4)

ff6_5 <- felm(fwd_car_5d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_5)

ff6_22 <- felm(fwd_car_22d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                 `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_22)

ff6_126 <- felm(fwd_car_126d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                  `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_126)

ff6_252 <- felm(fwd_car_252d_ff6_log ~ linear + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
                  `imbalance_in_-1d_100000_0` + car_1d_ff6_log + car_252d_ff6_log + car_126d_ff6_log + car_22d_ff6_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
summary(ff6_252)
...
#car1_500 <- felm(fwd_car_1d_log ~ linear*imbalance_in_0d_100000_0 + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
#               `imbalance_in_-1d_500_0` + car_1d_log + car_252d_log + car_126d_log + car_22d_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
#summary(car1_500)

#car2_500 <- felm(fwd_car_5d_log ~ linear*imbalance_in_1d_500_0 + estimate + prc_yest_log + f_rp_ess + f_rp_bmq + f_rp_bee + f_rp_bam + f_rp_bca + f_rp_css + f_rp_ber + f_rp_event_count +
#               `imbalance_in_-1d_500_0` + car_1d_log + car_252d_log + car_126d_log + car_22d_log + `vol_in_-1d_log` + marketcap_log | permno + ann_deemed_date + amaskcd | 0 | permno + ann_deemed_date, data = df_regression)
#summary(car2_500)

#analyze_felm_correlations(car2_500, df_regression)



#df_regression %>% group_by(bucket) %>% summarize(Size = mean(marketcap_log))
...
# Put CAR models in a list. Transform returns to basis points
#model_list <- list(`1`   = car1, `2`   = car2, `3`   = car3, `4`   = car4, `5`   = car5)
#coef_df <- bind_rows(lapply(names(model_list), function(h) extract_linear(model_list[[h]], as.numeric(h))))
#coef_df$beta <- 10000 * (coef_df$beta)
#coef_df$lower <- 10000 * (coef_df$lower)
#coef_df$upper <- 10000 * (coef_df$upper)
#coef_df$horizon <- as.numeric(coef_df$horizon)

# Function to extract coef and 90% CI for "linear" from one model
#extract_linear <- function(model, horizon_label) {
  coefs <- coef(model)
  ...
  # 90% two-sided CI -> 5% in each tail
  z     <- qnorm(0.95)
...
# Plot
#plot(seq(1,length(model_list)), coef_df$beta, pch = 16, ylim = c(-3,3))


# Add 90% confidence intervals as vertical error bars
#arrows(x0 = seq(1,length(model_list)), y0 = coef_df$lower,
#       x1 = seq(1,length(model_list)), y1 = coef_df$upper,
#       angle = 90, code = 3, length = 0.05)

# Reference line at zero
#abline(h = 0, lty = 2)

# Optional: use nice axis labels matching your horizons
#axis(1, at = x, labels = levels(coef_df$horizon))
